...
from app.services.ai_video.retry_policy import (
    MAX_AUTO_RETRIES,
    delete_archived_task_result_files,
...
def _probe_video_geometry(path: Path) -> tuple[int, int] | None:
    try:
        completed = subprocess.run(
            [
                FFPROBE_BIN if Path(FFPROBE_BIN).exists() else "ffprobe",
                "-v",
                "error",
                "-select_streams",
                "v:0",
    ...
    if completed.returncode != 0:
        return None
    try:
        streams = list(json.loads(completed.stdout.decode("utf-8")).get("streams") or [])
...
def _result_contract_error(
    ...
    for file in files:
        ...
        if path is None:
            continue
        if not (
            str(file.mime_type or "").lower().startswith("video/")
        ...
        if geometry is None:
            return (
                "local_result_invalid_video",
                f"Downloaded result {int(file.id)} has no readable video stream.",
            )
        width, height = geometry
        actual_value = width / height
        checked.append({"file_id": int(file.id), "width": width, "height": height})
        relative_error = abs(actual_value - expected_value) / expected_value
        if relative_error > OUTPUT_ASPECT_TOLERANCE:
            set_task_local_meta(
                task,
                output_contract={
            ...
            return (
                "video_output_aspect_mismatch",
                f"Requested video aspect ratio {expected}, but provider returned {width}x{height}.",
            )
    if checked:
        set_task_local_meta(
            task,
...
def _mark_success(db: Session, task: KieTask) -> dict[str, Any]:
    # The replacement becomes authoritative only after every local file and
    # output contract passed. Retire the previous successful generation at
    # this exact commit boundary, never when regeneration is merely requested.
    delete_archived_task_result_files(db, task)
    task.state = "success"
    task.updated_at = datetime.now()
    task.fail_code = None
    task.fail_msg = None
...
def queue_task_result_download(*, workspace_id: int, local_task_id: int, force: bool = False) -> None:
    ...
    try:
        download_task_result_files.apply_async(
            ...
            # Result transfer is isolated from provider submission/polling so
            # a slow RAID or upstream download cannot consume a generation
            # slot.  Recovery publishes into this same durable queue.
            queue=AI_VIDEO_DOWNLOAD_TASK_QUEUE,
    ...
    try:
        ...
        for candidate in tasks:
            ...
            if files and all(get_local_path(file) is not None for file in files):
                contract_error = _result_contract_error(task, files)
                if contract_error is not None:
                    _mark_failed(
                        db,
                        task,
                        code=contract_error[0],
                        message=contract_error[1],
                    )
                else:
    ...
    **_: Any,
    ...
    try:
        ...
        if current_files and not missing_files:
            contract_error = _result_contract_error(current_task, current_files)
            if contract_error is not None:
                return _mark_failed(
                    db,
                    current_task,
