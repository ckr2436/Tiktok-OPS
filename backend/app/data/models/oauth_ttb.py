...
class OAuthProviderApp(Base):
    ...
    service_id: Mapped[str | None] = mapped_column(String(128), default=None)
...
class OAuthTikTokAccountAuthzSession(Base):
    """OAuth state for TikTok account holder authorization."""

    __tablename__ = "oauth_tiktok_account_authz_sessions"
    __table_args__ = (
        UniqueConstraint("state", name="uk_tiktok_account_state"),
        Index("idx_tiktok_account_wid_status", "workspace_id", "status"),
        Index("idx_tiktok_account_expires_at", "expires_at"),
        Index("idx_tiktok_account_provider_app", "provider_app_id"),
    )
    ...
    return_to: Mapped[str | None] = mapped_column(String(512), default=None)
    alias: Mapped[str | None] = mapped_column(String(128), default=None)
    scopes_json: Mapped[dict | None] = mapped_column(JSON, default=None)
    ...
    status: Mapped[str] = mapped_column(
        Enum("pending", "consumed", "expired", "failed", name="oauth_tiktok_account_session_status"),
        nullable=False,
        server_default=text("'pending'"),
    )
    error_code: Mapped[str | None] = mapped_column(String(64), default=None)
...
class OAuthTikTokAccount(Base):
    """TikTok account holder token used by Accounts APIs such as video publish."""

    __tablename__ = "oauth_tiktok_accounts"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider_app_id", "open_id", name="uk_tiktok_account_open_id"),
        Index("idx_tiktok_accounts_wid_status", "workspace_id", "status"),
        Index("idx_tiktok_accounts_app", "provider_app_id"),
        Index("idx_tiktok_accounts_created_at", "created_at"),
    )
    ...
    creator_id: Mapped[str | None] = mapped_column(String(128), default=None)

    access_token_cipher: Mapped[bytes] = mapped_column(LargeBinary(4096), nullable=False)
    refresh_token_cipher: Mapped[bytes | None] = mapped_column(LargeBinary(4096), default=None)
    key_version: Mapped[int] = mapped_column(nullable=False, default=1)
    token_fingerprint: Mapped[bytes] = mapped_column(MySQL_BINARY(32), nullable=False)
    scope_json: Mapped[dict | None] = mapped_column(JSON, default=None)
    raw_json: Mapped[dict | None] = mapped_column(JSON, default=None)
    token_type: Mapped[str | None] = mapped_column(String(32), default=None)

    status: Mapped[str] = mapped_column(
        Enum("active", "revoked", "invalid", "expired", name="oauth_tiktok_account_status"),
        nullable=False,
        server_default=text("'active'"),
    )
