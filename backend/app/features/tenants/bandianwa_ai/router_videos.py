...
from app.services.kie_api.retry_policy import delete_task_result_files
...
from app.tasks.bandianwa.video_tasks import submit_and_poll_bandianwa_video_task
...
class BandianwaVideoTaskOut(BaseModel):
    id: int
    workspace_id: int
    model: str
    task_id: str
...
class BandianwaFileOut(BaseModel):
    id: int
    kind: str
    file_url: str
...
class BandianwaVideoBatchItem(BaseModel):
...
class BandianwaVideoCreateRequest(BandianwaVideoBatchItem):
    key_id: Optional[int] = None
    poll_interval_seconds: Optional[int] = None
    timeout_seconds: Optional[int] = None


class BandianwaVideoBatchRequest(BaseModel):
    items: List[BandianwaVideoBatchItem] = Field(..., min_length=1)
    key_id: Optional[int] = None
    poll_interval_seconds: Optional[int] = None
    timeout_seconds: Optional[int] = None


class BandianwaCreateResponse(BaseModel):
    task: BandianwaVideoTaskOut


class BandianwaBatchCreateResponse(BaseModel):
    tasks: List[BandianwaVideoTaskOut]
    total: int


class BandianwaTaskListResponse(BaseModel):
    items: List[BandianwaVideoTaskOut]
    total: int


class BandianwaClearTasksResponse(BaseModel):
    deleted_tasks: int
    deleted_files: int


class BandianwaTaskRegenerateRequest(BaseModel):
    input_params: Optional[dict[str, Any]] = None


class BandianwaBatchDownloadRequest(BaseModel):
    task_ids: List[int]
...
def _managed_cleanup_roots() -> tuple[Path, ...]:
    return (
        Path(settings.AI_VIDEO_RESULT_STORAGE_DIR).expanduser().resolve(),
        Path(settings.BANDIANWA_UPLOAD_STORAGE_DIR).expanduser().resolve(),
    )
...
def _safe_unlink(path_value: str | None) -> None:
    ...
    if not resolved.exists() or not resolved.is_file():
        return
    if not any(str(resolved).startswith(str(root)) for root in _managed_cleanup_roots()):
        return
    try:
...
def _normalize_service_provider(value: str | None) -> str:
    ...
    if provider in {"kyy", "keyiyun", "globalaiopc", "globalaiopc-omni-flash"}:
        return KYY_PROVIDER_KEY
    if provider in {"volcengine", "volcengine-ark", "ark"}:
        return VOLCENGINE_PROVIDER_KEY
    if provider in {"google", "gemini", "google-gemini", "google_gemini"}:
        return GOOGLE_GEMINI_PROVIDER_KEY
    if provider in {"bandianwa", "bdw"}:
        return BANDIANWA_PROVIDER_KEY
    if provider in {"toapis", "to-api", "to_api"}:
...
async def get_bandianwa_provider_status(
    workspace_id: int,
...
def _normalize_item(item: BandianwaVideoBatchItem) -> dict[str, Any]:
    ...
    if service_provider not in {
        "auto", "bandianwa", "kyy", "volcengine", "google-gemini", "google_gemini", "toapis"
    }:
    ...
    reference_limit = 9 if model == SEEDANCE_2_0_MINI_MODEL else 7
...
@router.post(
    "/generate",
    response_model=BandianwaCreateResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_bandianwa_video(
    workspace_id: int,
    payload: BandianwaVideoCreateRequest,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
):
    batch_payload = BandianwaVideoBatchRequest(
        key_id=payload.key_id,
        poll_interval_seconds=payload.poll_interval_seconds,
        timeout_seconds=payload.timeout_seconds,
        items=[
            BandianwaVideoBatchItem(
                model=payload.model,
    ...
    created = await create_bandianwa_video_batch(
        workspace_id=workspace_id,
        payload=batch_payload,
        me=me,
        db=db,
    )
    return BandianwaCreateResponse(task=created.tasks[0])


@router.post(
    "/batch",
    response_model=BandianwaBatchCreateResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_bandianwa_video_batch(
    workspace_id: int,
    payload: BandianwaVideoBatchRequest,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
):
    ...
    for item in payload.items:
        ...
        input_params["service_provider"] = str(key.provider_key)
    ...
    for task in tasks:
        try:
            ...
            logger.exception(
                "Failed to enqueue Bandianwa video task",
    ...
    return BandianwaBatchCreateResponse(tasks=tasks, total=len(tasks))
...
async def create_bandianwa_video_batch_upload(
    workspace_id: int,
    items: str = Form(...),
    ...
    parsed_items = [BandianwaVideoBatchItem.model_validate(raw) for raw in raw_items]
    ...
    for item in parsed_items:
        ...
        limit_for_item = 9 if normalized_model == SEEDANCE_2_0_MINI_MODEL else 7
    ...
    for task in tasks:
        try:
            ...
            logger.exception(
                "Failed to enqueue Bandianwa uploaded video task",
                extra={"workspace_id": workspace_id, "local_task_id": int(task.id)},
            )
            _mark_enqueue_failed(db, task, exc)
...
@router.get("/tasks", response_model=BandianwaTaskListResponse)
async def list_bandianwa_tasks(
    workspace_id: int,
    page: int = 1,
    size: int = 10,
    ...
    if refresh_pending:
        ...
        for item in items:
            ...
            try:
                ...
                db.rollback()
                logger.exception(
                    "Failed to refresh pending Bandianwa task",
    ...
    return BandianwaTaskListResponse(items=items, total=total)


@router.get("/admin/tasks", response_model=BandianwaTaskListResponse)
async def list_bandianwa_member_tasks(
    workspace_id: int,
    page: int = 1,
    size: int = 10,
    state: Optional[str] = None,
    ...
    if refresh_pending:
        ...
        for item in items:
            ...
            try:
                ...
                db.rollback()
                logger.exception(
                    "Failed to refresh pending Bandianwa member task",
                    extra={
                        "workspace_id": workspace_id,
                        "task_id": item.id,
                        "actor_user_id": int(me.id),
...
@router.delete("/tasks", response_model=BandianwaClearTasksResponse)
async def clear_bandianwa_tasks(
    workspace_id: int,
    model: Optional[str] = None,
    state: Optional[str] = None,
    me: SessionUser = Depends(require_tenant_member),
    ...
    if not task_ids:
        return BandianwaClearTasksResponse(deleted_tasks=0, deleted_files=0)
    ...
    return BandianwaClearTasksResponse(deleted_tasks=deleted_tasks, deleted_files=deleted_files)


@router.post("/tasks/{task_id}/retry", response_model=BandianwaCreateResponse)
async def retry_bandianwa_task(
    workspace_id: int,
    task_id: int,
    payload: Optional[BandianwaTaskRegenerateRequest] = None,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
):
    ...
    if override_input is None and str(task.fail_code or "").lower() == "local_download_failed":
        ...
        db.commit()
        db.refresh(task)
        queue_task_result_download(workspace_id=int(workspace_id), local_task_id=int(task.id))
        return BandianwaCreateResponse(task=task)

    delete_task_result_files(db, task)
    task = reset_bandianwa_task_for_retry(
        db,
        task=task,
...
@router.delete("/tasks/{task_id}", response_model=BandianwaClearTasksResponse)
async def delete_bandianwa_task(
    workspace_id: int,
    task_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
async def batch_download_bandianwa_tasks(
    workspace_id: int,
    payload: BandianwaBatchDownloadRequest,
    ...
    zip_path, filename, file_count = _create_batch_download_zip(
        rows,
        workspace_id=int(workspace_id),
        provider_label="bandianwa-videos",
    )
    background_tasks.add_task(_remove_temp_file, zip_path)
    logger.info(
        "Created Bandianwa batch video download",
        extra={
            "workspace_id": int(workspace_id),
            "actor_user_id": int(me.id),
...
@router.get("/admin/tasks/{task_id}", response_model=BandianwaVideoTaskOut)
async def get_bandianwa_member_task(
    workspace_id: int,
    task_id: int,
    refresh: bool = False,
    me: SessionUser = Depends(require_tenant_member),
    ...
    if refresh and _is_pending_state(task.state) and _supports_inline_refresh(task):
        try:
            ...
            db.rollback()
            logger.exception(
                "Failed to refresh Bandianwa member task",
                extra={
                    "workspace_id": workspace_id,
...
@router.get("/admin/tasks/{task_id}/files", response_model=List[BandianwaFileOut])
async def list_bandianwa_member_task_files(
    workspace_id: int,
    task_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
@router.get("/tasks/{task_id}", response_model=BandianwaVideoTaskOut)
async def get_bandianwa_task(
    workspace_id: int,
    task_id: int,
    refresh: bool = False,
    me: SessionUser = Depends(require_tenant_member),
    ...
    if refresh and _is_pending_state(task.state) and _supports_inline_refresh(task):
        try:
            ...
            db.rollback()
            logger.exception(
                "Failed to refresh Bandianwa task",
...
@router.get("/tasks/{task_id}/batch", response_model=List[BandianwaVideoTaskOut])
async def get_bandianwa_task_batch(
    workspace_id: int,
    task_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
@router.get("/tasks/{task_id}/files", response_model=List[BandianwaFileOut])
async def list_bandianwa_task_files(
    workspace_id: int,
    task_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
async def download_public_reference_file(
    ...
    if local_path is None:
        ...
        if not resolved.exists() or not resolved.is_file():
            raise HTTPException(status_code=404, detail="Reference file not found")
        allowed_roots = (
            Path(settings.GLOBALAIOPC_OMNI_FLASH_UPLOAD_STORAGE_DIR).expanduser().resolve(),
            Path(settings.BANDIANWA_UPLOAD_STORAGE_DIR).expanduser().resolve(),
            Path("/data/gmv_ops/hermes_content_factory").resolve(),
        )
...
async def get_bandianwa_file_download_url(
    workspace_id: int,
    file_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
async def download_bandianwa_file(
    workspace_id: int,
    file_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
async def get_bandianwa_member_file_download_url(
    workspace_id: int,
    file_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
...
async def download_bandianwa_member_file(
    workspace_id: int,
    file_id: int,
    me: SessionUser = Depends(require_tenant_member),
    db: Session = Depends(get_db),
