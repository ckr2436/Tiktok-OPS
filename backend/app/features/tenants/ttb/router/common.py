"""
Shared utilities, data models and helper functions for the TikTok Business
tenant API.

This module centralizes the definitions from the original monolithic
router file so that individual route modules can import what they need
without duplicating code.  Nothing in this file defines any FastAPI routes
directly.
"""
...
# Load all built‑in providers on import so that provider_registry is populated
load_builtin_providers()

# Supported provider identifiers for this feature.
...
def _relation_rank(value: Optional[str]) -> int:
    """Return a ranking integer for relation types."""
...
def _ensure_account(db: Session, workspace_id: int, auth_id: int) -> OAuthAccountTTB:
    """Ensure that an OAuth account exists and belongs to the given workspace."""
...
def _wait_for_run_completion(
    ...
    *,
    ...
    """Poll for a ScheduleRun to complete within the given timeout."""
...
def _extract_sync_summary(run: ScheduleRun) -> Optional[MetaSummary]:
    """Extract the sync summary from a ScheduleRun if present."""
    if not run or not run.stats_json:
        return None
    processed = run.stats_json.get("processed") or {}
    summary_block = processed.get("summary") or processed
...
def _serialize_binding(row: OAuthAccountTTB) -> ProviderAccount:
    """Serialize an OAuthAccountTTB into a ProviderAccount model."""
...
def _serialize_account_summary(row: OAuthAccountTTB) -> AccountSummary:
    """Serialize an OAuthAccountTTB into an AccountSummary model."""
...
def _normalize_if_none_match(value: Optional[str]) -> Optional[str]:
    """Normalize the If‑None‑Match header into a bare ETag token."""
    if not value:
        return None
...
) -> Tuple[MetaCursorState, bool]:
    """Poll the meta cursor state until it changes or a timeout expires."""
    deadline = time.monotonic() + timeout_seconds
    state = initial_state
    changed = False
    while time.monotonic() < deadline:
...
def _normalize_nullable_str(value: Any) -> Optional[str]:
    """Normalize a string value, returning None for empty or 'none' strings."""
    if value is None:
        return None
...
def _serialize_bc(row: TTBBusinessCenter) -> Dict[str, Any]:
    """Serialize a business center record."""
    raw = row.raw_json or {}
    ...
    status_val = _normalize_nullable_str(row.status) or _normalize_nullable_str(bc_info.get("status"))
    timezone_val = _normalize_nullable_str(row.timezone) or _normalize_nullable_str(bc_info.get("timezone"))
...
def _serialize_adv(row: TTBAdvertiser) -> Dict[str, Any]:
    """Serialize an advertiser record."""
    return {
        "advertiser_id": row.advertiser_id,
...
def _to_decimal(value: Any) -> Optional[Decimal]:
    """Safely coerce a value into a Decimal."""
    if value is None:
        return None
...
def _format_price_range(currency: Optional[str], low: Optional[Decimal], high: Optional[Decimal]) -> Optional[str]:
    """Format a low/high price range into a human‑readable string."""
...
def _extract_product_sku_count(raw: Optional[Dict[str, Any]]) -> Optional[int]:
    """Extract the SKU count from a product's raw JSON representation."""
...
def _extract_product_price_range(row: TTBProduct) -> Optional[str]:
    """Compute a user‑friendly price range string for a product."""
    ...
    if low_val is None and high_val is None:
        low_val = _to_decimal(row.price)
    return _format_price_range(currency, low_val, high_val)


def _extract_product_updated_time(row: TTBProduct) -> Optional[str]:
    """Extract the updated time for a product."""
...
def _serialize_product(
    ...
    return {
        ...
        "is_running_custom_shop_ads": (
            bool(row.is_running_custom_shop_ads) if row.is_running_custom_shop_ads is not None else None
        ),
        "ext_created_time": row.ext_created_time.isoformat() if row.ext_created_time else None,
        "ext_updated_time": row.ext_updated_time.isoformat() if row.ext_updated_time else None,
        "raw": row.raw_json,
...
def _coerce_utc(value: Optional[datetime]) -> Optional[datetime]:
    """Coerce a datetime into UTC timezone."""
    if not value:
        return None
...
def _get_business_center(
    ...
    """Retrieve a business center by its identifier."""
    row = (
...
def _get_advertiser(
    ...
    """Retrieve an advertiser by its identifier."""
    row = (
...
def _collect_bc_candidates(*values: Any) -> Set[str]:
    """Collect and normalize candidate business center identifiers from a list of values."""
...
def _resolve_advertiser_bc_candidates(
    ...
    """Return the set of BC IDs linked to an advertiser and the preferred BC based on relation priority."""
    rows = (
...
def _resolve_store_bc_candidates(
    ...
    """Return the set of BC IDs linked to a store and the preferred BC based on relation priority."""
    rows = (
        db.query(
...
def _validate_bc_alignment(
    *,
    ...
    """Validate that an advertiser and store belong to the same business center."""
...
def _meta_summary_from_dict(payload: Dict[str, Dict[str, int]]) -> MetaSummary:
    """Convert a raw summary dict into a MetaSummary model."""
...
def _perform_meta_sync(
    ...
    *,
    ...
    """Run a meta sync immediately and return its summary."""
    handler = provider_registry.get("tiktok-business")
    envelope = {
        "envelope_version": 1,
        "provider": "tiktok-business",
...
__all__ = [
    ...
    "_to_decimal",
    "_format_price_range",
    "_extract_product_sku_count",
    "_extract_product_price_range",
    "_extract_product_updated_time",
    "_serialize_product",
    "_coerce_utc",
    "_options_match",
    ...
    "_get_advertiser",
    "_get_store",
    "_collect_bc_candidates",
    "_resolve_advertiser_bc_candidates",
    "_resolve_store_bc_candidates",
