...
def default_scopes_for_provider(provider_key: str | None) -> list[str]:
    ...
    return [VIDEO_GENERATE_SCOPE, *(model_scope(model) for model in models)] if models else []


def default_model_priorities_for_provider(provider_key: str | None) -> dict[str, int]:
    provider = normalize_provider_key(provider_key)
    return dict(PROVIDER_DEFAULT_PRIORITIES.get(provider, {}))
...
def normalize_scopes(scopes: Iterable[str] | None, *, provider_key: str | None = None) -> list[str]:
    ...
    supported = supported_models_for_provider(provider)
    allowed = {VIDEO_GENERATE_SCOPE, *(model_scope(model) for model in supported)}
    result: list[str] = []
    for raw in source:
        value = str(raw or "").strip().lower().replace(" ", "_")[:128]
        if supported and value not in allowed:
            continue
...
def normalize_model_priorities(
    ...
    *,
    ...
    supported = set(supported_models_for_provider(provider))
    result: dict[str, int] = {}
...
def normalize_routing_config(
    *,
    ...
    enabled = {
        model for model in supported if model_scope(model) in set(normalized_scopes)
    }
    if not enabled:
        raise ValueError("At least one supported video model scope is required")
...
def key_scopes(key: KieApiKey) -> list[str]:
    # Provider capabilities are deployed code, not per-credential user input.
    return default_scopes_for_provider(key.provider_key)


def key_model_priorities(key: KieApiKey) -> dict[str, int]:
    defaults = default_model_priorities_for_provider(key.provider_key)
...
def legacy_route_is_eligible(db: Session, key: KieApiKey, model_id: str) -> bool:
    ...
    if not row.is_enabled or not row.is_verified:
        return False
    return not bool(row.circuit_open_until and row.circuit_open_until > datetime.utcnow())
            ...
            "routes": [
                {
                    ...
                    "capabilities": provider_model_capabilities(key.provider_key, model),
                    "reference_image_limit": provider_reference_limit(key.provider_key, model),
                }
                for key in routes
            ],
        })
