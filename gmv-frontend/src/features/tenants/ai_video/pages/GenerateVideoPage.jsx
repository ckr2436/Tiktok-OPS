...
import { useSessionQuery } from '../../../platform/auth/hooks.js'
import {
...
const SEEDANCE_PROMPT_MAX_LEN = 495
...
const FORM_FILE_DB_NAME = 'gmv_ai_video_form_state_v2'
...
function getLastTaskKey(wid, userId, modelId) {
  if (!wid || !userId) return ''
  return `${LAST_TASK_KEY_PREFIX}${wid}_u${userId}_${modelId}`
}

function getFormStateKey(wid, userId) {
  if (!wid || !userId) return ''
  return `${FORM_STATE_KEY_PREFIX}${wid}_u${userId}`
}
...
const SEEDANCE_SECONDS_OPTIONS = Array.from({ length: 12 }, (_, index) => index + 4).map((value) => ({
  value,
  label: `${value} 秒`,
}))
...
const MODEL_CONFIGS = [
  ...
  },
  {
    ...
    referenceImageLimit: 10,
  },
]

export function routingReferenceImageLimit(modelRoute, fallbackLimit) {
  const limits = (modelRoute?.routes || [])
    .map((route) => Number(route?.reference_image_limit))
    .filter((value) => Number.isInteger(value) && value > 0)
  return limits.length ? Math.max(...limits) : fallbackLimit
}

export function routingDurationOptions(modelRoute, fallbackOptions) {
  const routes = modelRoute?.routes || []
  if (!routes.length) return fallbackOptions
  if (routes.some((route) => !(route?.capabilities?.durations || []).length)) {
    return fallbackOptions
  }
  const durations = [...new Set(
    routes.flatMap((route) => route.capabilities.durations)
      .map(Number)
      .filter((value) => Number.isInteger(value) && value > 0),
  )].sort((left, right) => left - right)
  return durations.length
    ? durations.map((value) => ({ value, label: `${value} 秒` }))
    : fallbackOptions
}
...
export function shouldPollByState(state) {
  if (!state) return true
  const s = String(state).toLowerCase()
  // The backend may add intermediate states as provider routing evolves.
  // Poll every non-terminal state so a newly introduced state cannot silently
  // freeze the UI (``submitting`` previously did exactly that).
  return !TERMINAL_TASK_STATES.has(s)
}

function statusBadgeType(state) {
  return aiVideoTaskStatus(state).tone
}

export function taskStatusLabel(task) {
  return aiVideoTaskStatus(task).label
}
...
function toAiVideoAspectRatio(value) {
  if (value === 'landscape') return '16:9'
  if (value === 'square') return '1:1'
  return '9:16'
}

function createAiVideoDraft(seed = {}) {
  return {
...
async function deleteFormFileState(id) {
  if (!id) return
  const db = await openFormFileDb()
  if (!db) return
  await new Promise((resolve, reject) => {
    const tx = db.transaction(FORM_FILE_STORE, 'readwrite')
    tx.objectStore(FORM_FILE_STORE).delete(id)
    tx.oncomplete = resolve
    tx.onerror = () => reject(tx.error)
  })
  db.close()
...
export default function GenerateVideoPage() {
  ...
  const [aiVideoDrafts, setAiVideoDrafts] = useState(() => [
    createAiVideoDraft(),
  ])
  ...
  const currentModelRoute = modelRoutes[modelId]
  const referenceImageLimit = routingReferenceImageLimit(
    currentModelRoute,
    currentModel.referenceImageLimit || 7,
  )
  const durationOptions = routingDurationOptions(
    currentModelRoute,
    currentModel.kind === 'volcengine-seedance-mini'
      ? SEEDANCE_SECONDS_OPTIONS
      : BANDIANWA_SECONDS_OPTIONS,
  )
  const isModelAvailable = (id) => routingStatus == null || modelRoutes[id]?.available === true
  const promptMaxLength = modelId === 'seedance_2_0_mini'
    ? SEEDANCE_PROMPT_MAX_LEN
    : MAX_PROMPT_LEN
  ...
  const lastTaskKey = useMemo(
    () => getLastTaskKey(wid, currentUserId, modelId),
    [wid, currentUserId, modelId],
  )
  const formStateKey = useMemo(
    () => getFormStateKey(wid, currentUserId),
    [wid, currentUserId],
  )

  useEffect(() => {
    if (!wid || !currentUserId || typeof window === 'undefined') return
    const legacyFormKey = `${FORM_STATE_KEY_PREFIX}${wid}`
    window.localStorage.removeItem(legacyFormKey)
    for (const config of MODEL_CONFIGS) {
      window.localStorage.removeItem(`${LAST_TASK_KEY_PREFIX}${wid}_${config.id}`)
    }
    deleteFormFileState(legacyFormKey).catch(() => {})
  }, [wid, currentUserId])
  ...
  useEffect(() => {
    ...
    setAiVideoDrafts([createAiVideoDraft()])
    ...
    async function restoreFormState() {
      ...
      if (saved) {
        ...
        const restoredDrafts =
          Array.isArray(saved.aiVideoDrafts) && saved.aiVideoDrafts.length
            ? saved.aiVideoDrafts.map((item) => {
                const draft = createAiVideoDraft(item)
                return {
                  ...draft,
                  id: item.id || draft.id,
            ...
            : [createAiVideoDraft()]
        setAiVideoDrafts(restoredDrafts)
      }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const state = {
        modelId,
        pageSize,
        aiVideoDrafts: aiVideoDrafts.map((item) => ({
          id: item.id,
          prompt: item.prompt || '',
      ...
      saveFormFileState(formStateKey, {
        draftFiles: aiVideoDrafts.map((item) => ({
          draftId: item.id,
          files: (item.files || [])
            .filter((img) => img?.file)
  ...
  // 挂载 / 模型切换时恢复最近任务
  useEffect(() => {
    if (!wid || !lastTaskKey || typeof window === 'undefined') return
  ...
  useEffect(() => {
    ...
    if (lastTaskKey && typeof window !== 'undefined') {
  ...
  // -------- 上传文件相关 --------
  function updateAiVideoDraft(id, patch) {
    setAiVideoDrafts((list) =>
  ...
  function fromAiVideoAspectRatio(value) {
    if (value === '16:9') return 'landscape'
    if (value === '1:1') return 'square'
    return 'portrait'
  }

  function buildAiVideoItem(item) {
    const localImages = item.files || []

    return {
      model: modelId,
      prompt: (item.prompt || '').trim(),
      aspect_ratio: toAiVideoAspectRatio(item.aspectRatio),
      generate_audio: true,
      reference_images: [],
  ...
  function addAiVideoDraft(seed = null) {
    setAiVideoDrafts((list) => [
      ...list,
      createAiVideoDraft(
        seed || {
  ...
  function duplicateAiVideoDraft(item) {
    addAiVideoDraft({
      prompt: item.prompt,
  ...
  function removeAiVideoDraft(id) {
    setAiVideoDrafts((list) =>
      list.length <= 1 ? list : list.filter((item) => item.id !== id),
    )
  }

  function addAiVideoFiles(id, fileList) {
  ...
  function removeAiVideoFile(itemId, imageId) {
    setAiVideoDrafts((list) =>
      list.map((item) =>
        item.id === itemId
          ? {
  ...
  function removeAiVideoServerReferenceFile(itemId, imageId) {
    setAiVideoDrafts((list) =>
      list.map((item) => {
  ...
  function draftCapabilityError(item) {
    const serverReferenceCount = Math.max(
      (item.serverReferenceFiles || []).length,
      (item.serverReferenceFilePaths || []).length,
    )
    const referenceCount = (item.files || []).length + serverReferenceCount
    if (String(item.prompt || '').length > promptMaxLength) {
      return `当前 ${currentModel.label} 提示词最多 ${promptMaxLength} 个字符，系统不会静默截断`
    }
    if (referenceCount > referenceImageLimit) {
      return `当前启用的 ${currentModel.label} 路由最多支持 ${referenceImageLimit} 张参考图`
    }
    if (!durationOptions.some((option) => Number(option.value) === Number(item.seconds))) {
      return `当前启用的 ${currentModel.label} 路由不支持 ${item.seconds} 秒视频`
    }
    return ''
  }

  async function submitSingleAiVideoDraft(item) {
    ...
    if (!draft.prompt) {
      showErrorToast(`任务 ${aiVideoDrafts.findIndex((d) => d.id === item.id) + 1} 还没有填写提示词`)
      return
    }
    const capabilityError = draftCapabilityError(draft)
    if (capabilityError) {
      showErrorToast(capabilityError)
      return
    }
    const localImages = draft.files || []
    const payloadItem = buildAiVideoItem(draft)
  ...
  async function onSubmit(e) {
    ...
    const incompatibleDraftIndex = drafts.findIndex((item) => draftCapabilityError(item))
    if (incompatibleDraftIndex >= 0) {
      showErrorToast(`任务 ${incompatibleDraftIndex + 1}：${draftCapabilityError(drafts[incompatibleDraftIndex])}`)
      return
    }
    ...
    try {
      ...
      if (lastTask?.id) {
        ...
        if (lastTaskKey) window.localStorage.setItem(lastTaskKey, String(lastTask.id))
      }
      setPage(1)
      await refetchHistory()
    ...
    try {
      ...
      if (nextTask?.id) {
        ...
        if (typeof window !== 'undefined') {
          if (lastTaskKey) window.localStorage.setItem(lastTaskKey, String(nextTask.id))
        }
      }
      await refetchHistory()
      await refetchTask()
  ...
  async function loadAiVideoServerReferenceFiles(taskItem, input) {
    ...
    if (wid && taskItem?.id) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.warn('load AiVideo reference files failed', error)
      }
    }
  ...
  async function buildAiVideoDraftFromTask(taskItem) {
    const input = taskItem.input_json || {}
    const serverReferenceFiles = await loadAiVideoServerReferenceFiles(taskItem, input)
    return createAiVideoDraft({
      prompt: input.prompt || taskItem.prompt || '',
      aspectRatio: fromAiVideoAspectRatio(input.aspect_ratio),
  ...
  async function onEditRegenerateTaskItem(taskItem) {
    ...
    if (lastTaskKey) window.localStorage.setItem(lastTaskKey, String(taskItem.id))
    ...
    const drafts = await Promise.all(orderedTasks.map(buildAiVideoDraftFromTask))
    setAiVideoDrafts(drafts.length ? drafts : [await buildAiVideoDraftFromTask(taskItem)])
    setRegenerateTarget({
      id: taskItem.id,
  ...
  return (
    <div className="card card--elevated">
                ...
                {aiVideoDrafts.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                        <button
                          ...
                          onClick={() => submitSingleAiVideoDraft(item)}
                        >
                        ...
                        <button
                          ...
                          onClick={() => duplicateAiVideoDraft(item)}
                        >
                          复制
                        </button>
                        <button
                          type="button"
                          className="btn ghost sm"
                          disabled={aiVideoDrafts.length <= 1 || singleSubmittingDraftId === item.id}
                          onClick={() => removeAiVideoDraft(item.id)}
                        >
                          删除
                        </button>
                      </div>
                    ...
                    <FormField label="提示词">
                      ...
                      />
                      <div className="small-muted" style={{ marginTop: 4 }}>
                        {String(item.prompt || '').length}/{promptMaxLength} 字符
                        {modelId === 'seedance_2_0_mini'
                          ? '；豆包页面输入硬上限 500，系统按已验证的 495 字符安全预算提交。请把核心人物、动作和镜头放在前面；不会写入重复模式指令或静默截断。'
                          : ''}
                      </div>
                    </FormField>
                    ...
                    <FormField label={`参考图（拖拽或点击上传，最多 ${referenceImageLimit} 张）`}>
                        <div
                          ...
                          onDrop={(e) => {
                            e.preventDefault()
                            addAiVideoFiles(item.id, e.dataTransfer.files)
                          }}
                          style={{
                        ...
                        >
                          ...
                          >
                            ...
                            <input
                              ...
                              style={{ display: 'none' }}
                              onChange={(e) => {
                                addAiVideoFiles(item.id, e.target.files)
                                e.target.value = ''
                              }}
                            />
                          </label>
                              ...
                              {(item.serverReferenceFiles || []).map((img, imgIndex) => (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      removeAiVideoServerReferenceFile(item.id, img.id)
                                    }
                                    style={{
                                      position: 'absolute',
                                      right: 4,
                              ...
                              {item.files.map((img, imgIndex) => (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    className="btn danger sm"
                                    onClick={() =>
                                      removeAiVideoFile(item.id, img.id)
                                    }
                                    style={{
                                      position: 'absolute',
                                      right: 4,
            ...
            {task && !loadingTask && (
              <div className="card" style={{ marginTop: 4 }}>
                <div style={{ marginBottom: 8 }}>
                  ...
                  {aiVideoTaskProgressDetail(task) && (
                    <div className="small-muted" style={{ marginTop: 6 }}>
                      当前进度：{aiVideoTaskProgressDetail(task)}
                    </div>
                  )}
                  ...
                  {(task.fail_code || task.fail_msg) && (
                    ...
                    >
                      <summary style={{ cursor: 'pointer', fontWeight: 600 }}>查看失败原因</summary>
                        ...
                        <tr key={t.id} style={{ height: 54 }}>
                          ...
                          <td>
                            ...
                            >
                              ...
                              >
                                {isAiVideoTaskFailure(t) ? '查看原因' : '查看'}
                              </button>
                              {canRegenerateTask(t) && (
                                <button
                                  type="button"
      ...
      {failureDetail && (
        <Modal
          open
          title={`任务 #${failureDetail.id} 失败详情`}
        ...
        >
          ...
          >
            {failureDetail.fail_code && <>[{failureDetail.fail_code}] </>}
            {failureDetail.fail_msg || '未知错误'}
          </div>
        </Modal>
      )}
    </div>
