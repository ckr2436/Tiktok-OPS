...
import './PlatformAiProviderPage.css'
...
export default function PlatformAiProviderPage() {
  ...
  const roleGroupsQuery = useQuery({ queryKey: ['platform', 'ai-role-groups'], queryFn: apiKeyApi.listRoleGroups, staleTime: 15_000, refetchInterval: 60_000 })
  ...
  const refresh = async () => Promise.all([
    ...
    queryClient.invalidateQueries({ queryKey: ['platform', 'ai-role-groups'] }),
  ...
  const updateVideoDurations = (item, route, rawValue) => {
    const durations = [...new Set(String(rawValue || '')
      .split(/[^0-9]+/)
      .filter(Boolean)
      .map(Number)
      .filter((value) => Number.isInteger(value) && value >= 1 && value <= 300))]
      .sort((left, right) => left - right)
    const current = [...(route.capabilities?.durations || [])]
      .map(Number)
      .sort((left, right) => left - right)
    if (!durations.length) {
      setMessage({ type: 'error', text: '片段时长至少填写一个 1–300 秒的整数' })
      return
    }
    if (durations.join(',') === current.join(',')) return
    action.mutate({
      fn: () => apiKeyApi.updateRoute(route.id, { video_durations_seconds: durations }),
      success: `${route.key_name} ${item.model_label} 支持时长已更新`,
    })
  }

  const moveRoleProvider = (group, provider, direction) => {
    const order = [...(group.provider_order || [])]
    const index = order.indexOf(provider)
    const target = index + direction
    if (index < 0 || target < 0 || target >= order.length) return
    ;[order[index], order[target]] = [order[target], order[index]]
    action.mutate({
      fn: () => apiKeyApi.updateRoleProviderOrder(group.role, order),
      success: `${group.display_name} 的服务商顺序已更新`,
    })
  }

  const preferSub2Api = (group) => {
    const current = [...(group.provider_order || [])]
    const order = ['sub2api', ...current.filter((value) => value !== 'sub2api')]
    action.mutate({
      fn: () => apiKeyApi.updateRoleProviderOrder(group.role, order),
      success: `${group.display_name} 已设置为 Sub2API 优先`,
    })
  }

  const busy = keysQuery.isLoading || providersQuery.isLoading || overviewQuery.isLoading || roleGroupsQuery.isLoading
  const fatal = keysQuery.error || providersQuery.error || overviewQuery.error || roleGroupsQuery.error || catalogModelsQuery.error || routesQuery.error
  ...
  return (
    <div className="ai-admin">
      ...
      {!busy && tab === 'roles' && <>
        <div className="ai-role-intro">
          <div><strong>按业务管理实际调用链</strong><span>模型层级决定质量与速度；同一层级内按服务商顺序自动熔断切换。</span></div>
          <span className="ai-role-intro__primary">自建 Sub2API 为默认主线路</span>
        </div>
        <section className="ai-role-grid">
          {roleGroups.map((group) => {
            const active = group.active_route
            const tiers = [...new Set((group.routes || []).map((route) => route.source_logical_model_id))]
            return <article className="card ai-role-card" key={group.role}>
              <div className="ai-role-card__head">
                <div><h3>{group.display_name}</h3><span>{group.logical_model_id}</span></div>
                <Status value={active ? 'HEALTHY' : 'CIRCUIT_OPEN'} />
              </div>
              <p>{group.description || '由统一 AI 网关按健康状态自动选择供应商。'}</p>
              <div className="ai-role-card__facts">
                <div><span>模型层级</span><strong>{tiers.join(' → ') || '—'}</strong></div>
                <div><span>当前实际线路</span><strong>{active ? `${providerMap[active.provider_key] || active.provider_key} / ${active.provider_model_id}` : '暂无健康路由'}</strong></div>
              </div>
              <div className="ai-provider-order">
                <div className="ai-provider-order__label"><span>服务商优先级</span><button type="button" className="btn btn--sm" disabled={action.isPending || group.provider_order?.[0] === 'sub2api' || !group.provider_order?.includes('sub2api')} onClick={() => preferSub2Api(group)}>Sub2API 优先</button></div>
                {(group.provider_order || []).map((provider, index) => <div className={`ai-provider-order__row${index === 0 ? ' is-primary' : ''}`} key={provider}>
                  <span className="ai-provider-order__rank">{index === 0 ? '主' : index + 1}</span>
                  <strong>{providerMap[provider] || provider}</strong>
                  <span>{index === 0 ? '健康时优先调用' : '故障备用'}</span>
                  <div><button type="button" aria-label={`${group.display_name} ${provider} 上移`} className="btn btn--sm" disabled={action.isPending || index === 0} onClick={() => moveRoleProvider(group, provider, -1)}>↑</button><button type="button" aria-label={`${group.display_name} ${provider} 下移`} className="btn btn--sm" disabled={action.isPending || index === group.provider_order.length - 1} onClick={() => moveRoleProvider(group, provider, 1)}>↓</button></div>
                </div>)}
              </div>
              <div className="small-muted">{group.eligible_route_count || 0} 条健康候选 · {CAPABILITY_LABELS[group.capability] || group.capability} · {WORKLOAD_LABELS[group.workload] || group.workload}</div>
            </article>
          })}
          {!roleGroups.length && <div className="card empty">尚未物化业务角色路由，请先运行角色同步。</div>}
        </section>
      </>}
      ...
      {!busy && tab === 'routes' && <>
        ...
        {!!providerModelsQuery.data?.length && <section className="card ai-section">
          <div className="card__header">视频供应商能力与调用优先级 <span className="small-muted">每个 Key 独立配置优先级和片段时长；项目会先生成可跨供应商执行的时长合同。</span></div>
          <div className="card__body table-wrapper"><table className="table"><thead><tr><th>供应商</th><th>视频模型</th><th>输入上限</th><th>Key 路由优先级与片段时长</th><th>全局状态</th></tr></thead><tbody>
            {providerModelsQuery.data.map((item) => <tr key={`${item.provider_key}:${item.model_id}`}>
              <td>{item.provider_label}</td>
              <td>{item.model_label}</td>
              <td>最多 {item.reference_image_limit} 张参考图</td>
              <td><div className="ai-video-route-priorities">{(item.routes || []).map((route) => <label key={route.id}>
                <span>{route.key_name}</span>
                <input className="input ai-priority" aria-label={`${route.key_name} ${item.model_label} 优先级`} type="number" min="1" max="9999" defaultValue={route.priority} onBlur={(event) => { const value = Number(event.target.value); if (value !== route.priority) action.mutate({ fn: () => apiKeyApi.updateRoute(route.id, { priority: value }), success: `${route.key_name} 视频优先级已更新` }) }} />
                <input className="input" aria-label={`${route.key_name} ${item.model_label} 支持片段时长`} defaultValue={(route.capabilities?.durations || []).join(', ')} placeholder="如：4, 6, 8, 10" onBlur={(event) => updateVideoDurations(item, route, event.target.value)} />
                <small className="small-muted">秒，逗号分隔</small>
              </label>)}{!(item.routes || []).length && <span className="small-muted">尚无可配置路由</span>}</div></td>
              <td><button type="button" className={`btn btn--sm${item.is_enabled ? '' : ' btn--danger'}`} onClick={() => toggleVideoProvider(item)}>{item.is_enabled ? '已启用' : '已停用'}</button></td>
            </tr>)}
          </tbody></table></div>
        </section>}
      </>}
